package com.example.applewalletorderchangeanim.ui.reorderable
...
fun rememberReorderableLazyListState(
    ...
    val orientation by remember {
        derivedStateOf { lazyListState.layoutInfo.orientation }
    }
    val shouldItemMoveThresholdPx = with(density) { shouldItemMoveThreshold.toPx() }
    val state = remember(
        scope,
        lazyListState,
        scrollThreshold,
        ...
        ReorderableLazyListState(
            ...
            shouldItemMove = when (orientation) {
                Orientation.Vertical -> { draggingItem, item ->
                    (item.top + shouldItemMoveThresholdPx) in draggingItem.top..<draggingItem.bottom
                }
...
fun LazyItemScope.ReorderableItem(
    ...
    val orientation by remember(state.orientation) {
        derivedStateOf { state.orientation }
    }
    val dragging by state.isItemDragging(key)
    val offsetModifier = if (dragging) {
        // REMOVED zIndex here
        Modifier.then(when (orientation) {
